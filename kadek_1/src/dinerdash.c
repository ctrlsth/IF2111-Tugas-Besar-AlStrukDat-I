...
void dinerDASH(){
    ...
    while(!ended){
        ...
        system("cls");
        while(!EndKata){
            InsertLast(&Command,CKata);
            ADVKATA();
        }
        if(Command.Neff == 2){
            if(compareWord(Command.A[0],"COOK")){
                Cook(qPesanan,&qDiMasak,Command.A[1],&turn_end);
            }
            else if(compareWord(Command.A[0],"SERVE")){
                serve(&qDiMasak,&qSelesai,&qPesanan,Command.A[1],&saldo,&turn_end);
                if(turn_end){
                    count +=1;
                }
            }
            ...
            if(compareWord(Command.A[0],"SKIP")){
                turn_end = true;
            }
            else{
                printf("Command tidak valid\n");
